...
- **`POST /assets/:id/transfer`** — transferência é "devolução + nova alocação" num único passo: encerra a alocação ativa atual (se houver) com `returnDate = transferDate` e abre uma nova no destino informado (`assignedToName`, `siteId`, `departmentId`). Evita ter que devolver e alocar de novo manualmente quando um ativo muda de obra/filial, e registra o movimento como `TRANSFERENCIA` (não como `DEVOLUCAO` + `ENTREGA` separados) para o histórico não sugerir que o equipamento passou por estoque.
...
**Nota de implementação:** `allocate`, `returnAsset`, `transfer`, `sendToMaintenance` e `returnFromMaintenance` faziam suas 2-3 escritas dentro de uma `$transaction` interativa do Prisma. Isso foi removido (viraram sequências simples de `await this.prisma...`) pelo mesmo motivo documentado na seção 3.2 sobre a importação de extrato: contra um Postgres serverless com pooler (Neon), uma transação interativa pode ter sua conexão dedicada trocada/derrubada no meio do caminho, gerando `Transaction API error: Transaction not found`. Como nenhuma dessas ações precisa reverter parcialmente em caso de erro no meio (a pior consequência de uma falha a meio caminho é um estado intermediário consistente, não corrompido), abrir mão da atomicidade "tudo ou nada" para ganhar confiabilidade contra o pooler foi a troca certa aqui.

#### Status "Devolvido" para ativos locados e edição do responsável

Pedido do usuário: poder trocar o colaborador responsável por um ativo já em uso sem precisar de uma "transferência" completa (útil quando os colaboradores trocam de equipamento entre si, às vezes sem saber ainda quem ficou com qual), e ter um status que distinga um ativo **locado devolvido à locadora** de um ativo simplesmente "em estoque":

- **`PATCH /assets/:id/assigned-to`** (`UpdateAssignedToDto`) — corrige só o nome do responsável na alocação ativa atual, sem fechar/reabrir alocação, sem mexer em site/departamento e sem gerar uma nova linha de movimentação, porque fisicamente nada mudou de lugar. Pode vir em branco (`""`) quando ainda não se sabe quem ficou com o equipamento — nesse caso o serviço normaliza para `"Não informado"` em vez de gravar uma string vazia. Diferente de `transfer`, que é para quando o ativo *fisicamente* muda de mãos/obra.
- **Novo valor no enum `AssetStatus`: `DEVOLVIDO`.** Antes, devolver qualquer ativo (próprio ou locado) jogava o status para `ESTOQUE`. Isso é correto para um ativo **PRÓPRIO** recolhido (ele continua seu, disponível para realocar), mas estava errado para um ativo **LOCADO** devolvido à locadora: fisicamente ele não está mais com a empresa, então tratá-lo como "estoque disponível" é enganoso — e pior, `AssetsService.findIdle()` (que alimenta o card "Equipamentos Ociosos" do Dashboard) considera `status: ESTOQUE, ownership: LOCADO` como ocioso gerando custo, então um locado devolvido aparecia incorretamente como "ocioso" cobrando aluguel que na verdade não existe mais. `returnAsset()` agora decide o status de destino pela propriedade do ativo (`ownership === LOCADO ? DEVOLVIDO : ESTOQUE`), o que resolve isso na raiz sem precisar tocar em `findIdle()`.
- **Como isso interage com a fatura do mês seguinte:** o sistema não "desconta" nada de uma fatura ao marcar um ativo como devolvido — as faturas (`Invoice.grossValue`) vêm sempre do extrato real que a locadora envia todo mês (seção 3.2). Uma vez devolvido, o equipamento simplesmente vai deixar de aparecer no próximo PDF importado, e a fatura daquele mês em diante já vem sem o custo dele — o status `DEVOLVIDO` no cadastro é só para refletir/registrar isso no sistema (e permitir consultar o histórico completo do ativo depois), não para calcular a fatura. Se um ativo marcado como `DEVOLVIDO` aparecer de novo num extrato futuro (a locadora ainda cobrando, ou devolução não processada), a importação (que usa o extrato como fonte da verdade) volta o status para `EM_USO` normalmente.
- Um ativo `DEVOLVIDO` continua com todo o histórico de alocações/movimentações consultável em `GET /assets/:id` — ele só sai das ações de "Alocar" e "Enviar para manutenção" na tela de Ativos, já que não faz sentido realocar ou mandar para manutenção um equipamento que não está mais fisicamente com a empresa.
